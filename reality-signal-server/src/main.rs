...
async fn main() {
    ...
    let routes = chat.or(root_redirect).or(static_files)
        ...
        .with(warp::reply::with::header("Content-Security-Policy", "default-src 'self'; script-src 'self' 'unsafe-inline' 'wasm-unsafe-eval' https://unpkg.com; style-src 'self' 'unsafe-inline' https://unpkg.com; connect-src 'self' https://unpkg.com ws://localhost:9000 wss://localhost:9000 http://localhost:9000 https://localhost:9000 https://0.peerjs.com wss://0.peerjs.com; img-src 'self' data: blob:; media-src 'self' blob:;"))
