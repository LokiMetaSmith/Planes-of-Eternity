...
impl State {
    async fn new(canvas: HtmlCanvasElement) -> Self {
        ...
        let _half_grid = (grid_count as f32 * chunk_size) / 2.0;
        let _offset = chunk_size / 2.0; // Mesh is centered, so we move centers.
...
pub async fn start(canvas_id: String) -> Result<GameClient, JsValue> {
    ...
    // Mouse Handler
    let _state_mouse = state.clone();
    let _canvas_mouse = canvas.clone();
